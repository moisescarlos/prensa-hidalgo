...
Entretenimiento.propTypes = {
  ...
  export default () => (
    <StaticQuery
      query={graphql`
        query EntretenimientoQuery {
            allMarkdownRemark(sort: {order: DESC, fields: [frontmatter___date]}, filter: {frontmatter: {templateKey: {eq: "blog-post"}, category: {eq: "Entretenimiento"}}}) {
                nodes {
                  fields {
                    slug
                  }
      ...
      `}
      render={(data, count) => <Entretenimiento data={data} count={count} />}
    />
  )